 ...
 */
const chunkFiles = async (namespace: string) => {
  const fileContents: FileToEmbed[] = [];

  const files = fs.readdirSync(SHARED_DIR + "/" + namespace);
  try {
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const filePath = path.join(SHARED_DIR + "/" + namespace, file);
...
export const generateEmbeddings: GenerateEmbeddings<string, string> = async (namespace, context) => {
  try {
    const contentChunked = await chunkFiles(namespace);
    ...
    const pineconeIndex = pinecone.Index('embeds');

    console.log(`generateEmbeddings for ${namespace} [[ starting... ]] `);
...
export const embedSeedScript = async (prismaClient: typeof PrismaClient) => {
  await generateEmbeddings("DM", {
    entities: { TextChunk: prismaClient.textChunk, ParentFile: prismaClient.parentFile },
  });
  await generateEmbeddings("FA", {
    entities: { TextChunk: prismaClient.textChunk, ParentFile: prismaClient.parentFile },
  });
  await generateEmbeddings("FBA", {
    entities: { TextChunk: prismaClient.textChunk, ParentFile: prismaClient.parentFile },
  });
  await generateEmbeddings("SCM", {
    entities: { TextChunk: prismaClient.textChunk, ParentFile: prismaClient.parentFile },
  });
  await generateEmbeddings("SNA", {
    entities: { TextChunk: prismaClient.textChunk, ParentFile: prismaClient.parentFile },
  });
};
